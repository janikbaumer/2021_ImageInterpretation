...
if __name__ == "__main__":
    ...
    for x_tst, y_tst in tqdm(test_loader):  # if windowsize of val_loader is set to full size (10980), then length of this loop = batch_size of dataLoader
        ...
        for i in range(len(x_tst)):
            ...
            # compare prediction to random vector
            if not X_test.shape == (0,) and not Y_test.shape == (0,):
                Y_pred_random = np.random.randint(3, size=(Y_test.shape[0], 1))


                cm_random = confusion_matrix(Y_test, Y_pred_random, labels=CLASSES)
                CM_full_random = CM_full_random + cm_random


    print('CONFUSION MATRICES COMPUTED - SAVING CONFUSION MATRICES ...')

    np.savetxt('cm_full_random.csv', CM_full_random, delimiter=',')
